...
fun cps (Var(x), bindings) = 
   ...
   | cps (App(termM, termN), bindings) =
        let
            val fv_M = freeVariables(termM, bindings)
            val fv_N = freeVariables(termN, bindings)
            val k = genSymbol("k", fv_M @ fv_N)
            val n = genSymbol("n", fv_M)
            val m = genSymbol("m", [k, n])
            val cps_M = cps(termM, k::n::bindings)
            val cps_N = cps(termN, k::bindings)
        in
            Lambda(k, App(cps_N, Lambda(n, App(cps_M, Lambda(m, App(App(Var(m), Var(n)), Var(k)))))))
        end
